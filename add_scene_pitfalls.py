#!/usr/bin/env python3
"""为 calculators.js 中的每个公式添加 scene（使用场景）和 pitfalls（常见坑点）"""
...
SCENE_PITFALLS = {
    ...
    'bearing_weak': {
        ...
        'pitfalls': ['pz 是附加应力，不是总应力，需要先算 pz 再算 pcz', 'f_az 也需要做深度修正，不能直接用 fak']
    },
    'bearing_area': {
    ...
    'bearing_spread': {
        ...
        'pitfalls': ['矩形基础中心点用 4 倍角点法，角点用 1 倍，L/b 比值影响 α̅', 'α̅ 查表时 l/b 和 z/l 的比值容易搞反']
    },
    'bearing_eccentric': {
    ...
    'pile_ra': {
        ...
        'pitfalls': ['安全系数 2 不能忘（Ra=Quk/2）', 'upsilon_p=π·d，桩径 d 单位是 m 不是 mm', 'qsik、qpk 查表值与桩型（预制/灌注）有关']
    },
    'pile_quk': {
    ...
    'pile_horiz': {
        'scene': '桩承受水平荷载（如桥桩、 offshore 桩）时的承载力',
    ...
    'treat_replace_m': {
        ...
        'pitfalls': ['de 等效影响直径：正方形布桩 de=1.13s，等边三角形 de=1.05s', 'm 通常 0.05~0.35，超出范围需检查']
    },
    'treat_prelod_settle': {
    ...
    'treat_compact_density': {
        ...
        'pitfalls': ['ρd1 是桩体干密度，ρd2 是桩间土挤密后干密度', '处理后的干密度需 ≥ 压实系数 × 最大干密度']
    },
    'treat_replace_thickness': {
        'scene': '换填垫层底面宽度计算，用于下卧层验算的应力扩散',
        'pitfalls': ['θ 是垫层的应力扩散角，砂石 θ=30°，粉质黏土 θ=20°~25°', 'z 是垫层厚度，不是基础埋深', 'bz 算出后用于计算下卧层顶面附加应力']
    },
    'slope_infinite': {
    ...
    'slope_active': {
        ...
        'pitfalls': ['Ka 与 φ 有关：Ka=tan²(45°-φ/2)，φ 取有效内摩擦角', '有地下水时需另算静水压力，叠加', '墙后填土有超载 q 时：Ea=0.5γH²Ka + qH·Ka']
    },
    'slope_anchor': {
    ...
    'slope_overturn': {
        'scene': '重力式挡墙抗倾覆稳定性验算',
        'pitfalls': ['Ko≥1.6 是规范要求（比抗滑移更严格）', 'MR 绕墙趾计算，MO 也绕墙趾计算', '力矩臂容易算错，需仔细看图']
    },
    'slope_passive': {
    ...
    'slope_water_press': {
        ...
        'pitfalls': ['γw=10kN/m³，不是 9.8', 'hw 是水头高度（水面到计算点的垂直距离）', '土压力和静水压力需分别计算后叠加']
    },
    'excav_atc': {
    ...
    'excav_anchor_len': {
        'scene': '锚杆锚固段长度设计',
        'pitfalls': ['[σ] 是锚固体与土体的粘结强度设计值，单位 MPa 需×1000 转为 kPa', 'η 是锚杆工作条件系数', 'df 是锚固体直径（不是钢筋直径）']
    },
    'excav_pile_moment': {
        'scene': '排桩最大弯矩简化估算',
    ...
    'special_rock_fall': {
        'scene': '危岩体平面滑动稳定性分析',
        'pitfalls': ['W 是危岩体自重（需先算体积×重度）', 'α 是滑动面倾角，φ 是滑动面摩擦角', 'c、φ 是滑动面的参数，不是岩体的参数']
    },
...
if __name__ == '__main__':
    ...
    print(f"Total formulas with scene/pitfalls: {len(SCENE_PITFALLS)}")
    for k, v in SCENE_PITFALLS.items():
        print(f"  {k}: {len(v['pitfalls'])} pitfalls")
